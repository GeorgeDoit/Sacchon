     ...
     public ApplicationUser findById(String username) throws SQLException {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                user.setUsername(rs.getString(Constants.QV_USERNAME));
                user.setPassword(rs.getString(Constants.QV_PASSWORD));
                user.setRole(SacchonRole.getRoleValue(rs.getString("role")));
                return user;
            }
            return null;
        } finally {
